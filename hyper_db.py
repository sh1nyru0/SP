...
import pandas as pd
import pymysql
from PyQt5.QtWidgets import QMessageBox, QVBoxLayout, QHBoxLayout, QCheckBox, QSpacerItem, QSizePolicy, QLineEdit, \
    QWidget, QPushButton, QDialogButtonBox, QFormLayout, QDialog
...
class HyperDB:
    def __init__(self, connection):
        ...
        self.page = 0
        self.columns_names = ['id', 'Filename', 'content', 'description', 'samples', 'lines', 'bands', 'type', 'len']
    ...
    def init(self):
        ...
        sql = 'SELECT * FROM hyper_data LIMIT %s'
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM hyper_data WHERE {conditions_str}'
        cursor.execute(sql, tuple(conditions.values()))
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(data.shape[1], 9)
        df = pd.DataFrame(data, columns=self.column_names)
        df = df.apply(pd.to_numeric, errors='ignore')
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE hyper_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM hyper_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def down(self):
        cursor = self.connection.cursor()
        sql = 'SELECT COUNT(*) FROM hyper_data'
        rowcount = cursor.execute(sql)
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM hyper_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From hyper_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 9)
