...
import numpy as np
import pandas as pd
from PyQt5.QtWidgets import QLineEdit, QVBoxLayout, QFormLayout, QDialogButtonBox, \
    QDialog, QMessageBox, QCheckBox, QHBoxLayout, QWidget, QSizePolicy, QSpacerItem, QPushButton
...
class GravityDB:
    def __init__(self):
        ...
        self.column_names = ['id','Filename', 'Line_no', 'Flight_ID', 'Lon', 'Lat', 'x', 'y', 'Height_WGS1984', 'Date',
                             ...
                             'rec_grav', 'Abs_grav', 'VaccCor', 'EotvosCor', 'FaCor', 'HaccCor', 'Free_air', 'FAA_filt',
                             'FAA_clip', 'Level_cor', 'FAA_level', 'Fa_4600m']
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM gravity_data WHERE {conditions_str}'
        cursor.execute(sql, tuple(conditions.values()))
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(data.shape[1], 32)
        df = pd.DataFrame(data, columns=self.column_names)
        df = df.apply(pd.to_numeric, errors='ignore')
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE gravity_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM gravity_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def add(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            cursor = self.connection.cursor()
            sql = 'INSERT INTO gravity_data(Filename, Line_no, Flight_ID, Lon, Lat, x, y, Height_WGS1984, Date, Time, ST, CC,' \
                  'RB, XACC, LACC, Still, Base, ST_real, Beam_vel, rec_grav, Abs_grav, VaccCor, EotvosCor, FaCor, HaccCor, Free_air,' \
                  'FAA_filt, FAA_clip, Level_cor ,FAA_level, Fa_4600m) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,' \
    ...
    def down(self):
        ...
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM gravity_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From gravity_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 32)
