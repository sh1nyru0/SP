...
class MagneticDB:
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM magnetic_data WHERE {conditions_str}'
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE magnetic_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM magnetic_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def down(self):
        cursor = self.connection.cursor()
        sql = 'SELECT COUNT(*) FROM magnetic_data'
        rowcount = cursor.execute(sql)
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM magnetic_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From magnetic_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 20)
