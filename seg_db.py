...
import pandas as pd
import pymysql
from PyQt5.QtWidgets import QMessageBox, QHBoxLayout, QVBoxLayout, QCheckBox, QSpacerItem, QLineEdit, QSizePolicy, \
...
class SegDB:
    def __init__(self, connection):
        ...
        self.page = 0
        self.column_names = ['id', 'Filename', 'opnum', 'olnum', 'ns', 'dt', 'e', 'n', 'ampl']
    ...
    def init(self):
        ...
        sql = 'SELECT * FROM seg_data LIMIT %s'
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM seg_data WHERE {conditions_str}'
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE seg_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM seg_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def down(self):
        cursor = self.connection.cursor()
        sql = 'SELECT COUNT(*) FROM seg_data'
        rowcount = cursor.execute(sql)
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM seg_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From seg_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 9)
