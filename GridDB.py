...
class GridBD:
    def __init__(self):
        ...
        self.page = 0
        self.column_names = ['id', 'Filename', 'nr', 'nc', 'nx', 'ny', 'nz']
    ...
    def init(self):
        ...
        sql = 'SELECT * FROM grid_data LIMIT %s'
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM grid_data WHERE {conditions_str}'
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE sar_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM sar_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def add(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            cursor = self.connection.cursor()
            sql = 'INSERT INTO grid_data(Filename, nr, nc, nx, ny, nz) VALUES (%s, %s, %s, %s, %s, %s)'
            items = []
            for item in inputs:
    ...
    def down(self):
        cursor = self.connection.cursor()
        sql = 'SELECT COUNT(*) FROM grid_data'
        rowcount = cursor.execute(sql)
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM grid_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From grid_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 7)
