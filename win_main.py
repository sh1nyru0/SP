...
from FcnnWin import FcnnWin
from GridWin import GridWin
from KmeansWin import KmeansWin
from PredictionWin import PredictionWin
from editPluginWin import EditPluginWin
from lstmWin import LstmWin
from dnnWin import DnnWin
from db import DB
from libs.share import MySignals, SI
from original_data import Original_Data
from pandas_model import PandasModel
from plugin_manger import PluginMangerWin
...
electrolist = ['Filename', 'opnum', 'freq', 'comp', 'ampa', 'emag', 'ephz', 'hmag', 'hphz', 'resistivity', 'phase',
               'rho', 'phz', 'xq', 'yq', 'zq', 'gridname']
gamalist = ['Filename', 'Lon', 'Lat', 'kc', 'thc', 'uc', 'xq', 'yq', 'zq', 'gridname']
gravitylist = ['Filename', 'Line_no', 'Flight_ID', 'Lon', 'Lat', 'x', 'y', 'Height_WGS1984', 'Date', 'Time', 'ST', 'CC',
               'RB', 'XACC', 'LACC', 'Still', 'Base', 'ST_real', 'Beam_vel', 'rec_grav', 'Abs_grav', 'VaccCor',
               'EotvosCor', 'FaCor', 'HaccCor', 'Free_air', 'FAA_filt', 'FAA_clip', 'Level_cor', 'FAA_level',
               'Fa_4600m', 'xq', 'yq', 'zq', 'gridname']
hyperlist = ['Filename', 'content', 'description', 'samples', 'lines', 'bands', 'type', 'len', 'xq', 'yq', 'zq',
             'gridname']
magneticlist = ['Filename', 'Line_name', 'point', 'lon', 'lat', 'x', 'y', 'Height_WGS1984', 'Date', 'MagR', 'Magc',
                'RefField', 'MagRTC', 'BCorr', 'MagBRTC', 'ACorr', 'MagF', 'MagL', 'MagML', 'MagML_Drape', 'xq', 'yq',
                'zq', 'gridname']
...
_gpslist = ['Filename', 'FH', 'UCTCT', 'Lon', 'Lat', 'Vel', 'UTCD']
_electrolist = ['Filename', 'opnum', 'freq', 'comp', 'ampa', 'emag', 'ephz', 'hmag', 'hphz', 'resistivity', 'phase',
                'rho', 'phz']
_gamalist = ['Filename', 'Lon', 'Lat', 'kc', 'thc', 'uc']
_gravitylist = ['Filename', 'Line_no', 'Flight_ID', 'Lon', 'Lat', 'x', 'y', 'Height_WGS1984', 'Date', 'Time', 'ST',
                'CC', 'RB', 'XACC', 'LACC', 'Still', 'Base', 'ST_real', 'Beam_vel', 'rec_grav', 'Abs_grav', 'VaccCor',
                'EotvosCor', 'FaCor', 'HaccCor', 'Free_air', 'FAA_filt', 'FAA_clip', 'Level_cor', 'FAA_level',
                'Fa_4600m']
_hyperlist = ['Filename', 'content', 'description', 'samples', 'lines', 'bands', 'type', 'len']
_magneticlist = ['Filename', 'Line_name', 'point', 'lon', 'lat', 'x', 'y', 'Height_WGS1984', 'Date', 'MagR', 'Magc',
                 'RefField', 'MagRTC', 'BCorr', 'MagBRTC', 'ACorr', 'MagF', 'MagL', 'MagML', 'MagML_Drape']
_sarlist = ['Filename', 'content', 'size', 'type', 'direction', 'proj']
_seglist = ['Filename', 'opnum', 'olnum', 'ns', 'dt', 'e', 'n', 'ampl']
_lists = ['-', _gpslist, _sarlist, _seglist, _gamalist, _electrolist, _magneticlist, _gravitylist, _hyperlist]
datanames = ['-', 'gps_data', 'sar_data', 'seg_data', 'gama_data', 'electro_data', 'magnetic_data', 'gravity_data',
             'hyper_data']
cdatanames = ['-', 'gps数据', '地基合成孔径雷达数据', '地震数据', '放射性数据', '电法数据', '磁法数据', '重力数据',
              '高光谱数据']
...
class Win_Filter:
    ...
    def chooseDataBase(self, index):
        ...
        if index > 0:
            for i, name in enumerate(_lists[index]):
                container = QWidget()
                layout = QHBoxLayout()
                label = QLabel(name)
                cb = QComboBox()
    ...
    def onLoadData(self):
        ...
        for i in range(self.df.shape[0]):
            ...
            values = [rowdata[self.cbs[j].currentText()] for j in range(len(self.cbs))]
        ...
        SI.mainWin._openSubWin(DB, connection=self.connection, tableid=index, path=path)
        # 在数据表里创建文件夹并且刷新文件树
...
class Win_Main:
    def __init__(self, projectpath):
        ...
        self.ui.actionLstm.triggered.connect(self.onLstm)
        self.ui.actionKmeans.triggered.connect(self.onKmeans)
        self.ui.actionDnn.triggered.connect(self.onDnn)
        self.ui.actionFcnn.triggered.connect(self.onFcnn)
        self.ui.actionPrediction.triggered.connect(self.onPrediction)
        self.ui.actionPlugin.triggered.connect(self.editPlugin)
        self.ui.actionPluginManger.triggered.connect(self.pluginManger)
    ...
    def pluginManger(self):
        SI.pluginManagerWin = PluginMangerWin()
        SI.pluginManagerWin.ui.show()

    def editPlugin(self):
        text, ok = QInputDialog.getText(QWidget(), "输入插件类的名称",
                                        "请输入插件类的名称:", QLineEdit.Normal, "")
        if ok:
            SI.editPluginWin = EditPluginWin(text)
            SI.editPluginWin.ui.show()

    def onPrediction(self):
        SI.precdictionWin = PredictionWin(self.projectpath)
        SI.precdictionWin.ui.show()

    def onDnn(self):
        SI.dnnWin = DnnWin(self.projectpath)
        SI.dnnWin.ui.show()

    def onFcnn(self):
        SI.fcnnWin = FcnnWin(self.projectpath)
        SI.fcnnWin.ui.show()
    ...
    def onKmeans(self):
        ...
        #     data = pickle.load(f)
        #     print(data)
        SI.kmeansWin = KmeansWin(self.projectpath)
        SI.kmeansWin.ui.show()

    def onLstm(self):
        SI.lstmWin = LstmWin(self.projectpath)
        SI.lstmWin.ui.show()
    ...
    def _openSubWin(self, FuncClass, connection=None, tableid=None, path=None):
        def createOrUpdateSubWin():
            # 创建新子窗口实例
            subWinFunc = FuncClass(connection, tableid, path)
    ...
    def onItemDoubleClicked(self, item):
        ...
        if not os.path.isdir(path):
            ...
            if (os.path.basename(path) == "重力数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=7, path=pdir)
            if (os.path.basename(path) == "磁法数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=6, path=pdir)
            if (os.path.basename(path) == "电法数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=5, path=pdir)
            if (os.path.basename(path) == "地震数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=3, path=pdir)
            if (os.path.basename(path) == "gps数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=1, path=pdir)
            if (os.path.basename(path) == "放射性数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=4, path=pdir)
            if (os.path.basename(path) == "高光谱数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=8, path=pdir)
            if (os.path.basename(path) == "地基合成孔径雷达数据"):
                SI.mainWin._openSubWin(DB, connection=self.connection, tableid=2, path=pdir)
            if (os.path.basename(path) == "模型"):
                pass
            else:
                if (pdir == "重力数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=7, path=path)
                if (pdir == "磁法数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=6, path=path)
                if (pdir == "电法数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=5, path=path)
                if (pdir == "地震数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=3, path=path)
                if (pdir == "gps数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=1, path=path)
                if (pdir == "放射性数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=4, path=path)
                if (pdir == "高光谱数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=8, path=path)
                if (pdir == "地基合成孔径雷达数据"):
                    SI.mainWin._openSubWin(DB, connection=self.connection, tableid=2, path=path)
