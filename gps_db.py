...
class GpsDB:
    def __init__(self):
        ...
        self.page = 0
        self.column_names = ['id', 'Filename', 'FH', 'UCTCT', 'Lon', 'Lat', 'Vel', 'UTCD']
    ...
    def init(self):
        ...
        sql = 'SELECT * FROM gps_data LIMIT %s'
        cursor.execute(sql, (500))
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 8)
    ...
    def on_ok_clicked(self):
        ...
        cursor = self.connection.cursor()
        sql = f'SELECT {columns_str} FROM gps_data WHERE {conditions_str}'
        cursor.execute(sql, tuple(conditions.values()))
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(data.shape[1], 8)
        df = pd.DataFrame(data, columns=self.column_names)
        df = df.apply(pd.to_numeric, errors='ignore')
        ...
        if self.model.is_dirty():
            ...
            for row, col in dirty_cells:
                ...
                if choice == QMessageBox.No:
                    ...
                    sql = f'UPDATE gps_data SET {column_name} = %s WHERE id = %s'
    ...
    def delete(self):
        ...
        for row in rows:
            ...
            ids.append(row_data)
        cursor = self.connection.cursor()
        sql = "DELETE FROM gps_data WHERE id IN (%s)" % ','.join(['%s'] * len(ids))
    ...
    def down(self):
        cursor = self.connection.cursor()
        sql = 'SELECT COUNT(*) FROM gps_data'
        rowcount = cursor.execute(sql)
        if (rowcount / 500 > self.page):
            self.page += 1
        sql = 'SELECT * FROM gps_data LIMIT %s, %s'
    ...
    def first(self):
        cursor = self.connection.cursor()
        sql = 'SELECT * From gps_data LIMIT 0, 500'
        cursor.execute(sql)
        res = cursor.fetchall()
        data = np.array([res])
        data = data.reshape(500, 8)
