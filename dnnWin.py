...
class DnnWin:
    ...
    def ok(self):
        ...
        for i in range(self.levelNum):
            if i == 0:
                model.add(layers.Dense(int(self.arr1[i].toPlainText()), activation='relu', input_shape=(train_X.shape[1],)))
            else:
                model.add(layers.Dense(int(self.arr1[i].toPlainText()), activation='relu'))
